...
Video tidak membawa input. Backend BLE tidak mengetahui koordinat absolut jendela UxPlay dan tidak menerima orientasi layar untuk mengubah arah pointer secara otomatis. Pelacakan bentuk video pada launcher yang dijelaskan di bawah terpisah dari pemetaan input tersebut.
...
Mulai 0.6.0, launcher dapat mengubah **geometri** jendela video milik sesinya. Mulai 0.7.0, **Sematkan video di atas** juga dapat mengubah urutan Z jendela yang sama. Ini satu-satunya pengecualian dari prinsip pengamatan read-only, dan cakupannya sempit: enumerasi metadata jendela desktop bersifat read-only; `MirrorWindowLifecycle` hanya meneruskan kandidat class/title exact yang terbukti berada dalam Job receiver, lalu `MirrorWindowPositioner` memvalidasi ulang PID sebelum memanggil `SetWindowPos`/`SetWindowLongPtr` untuk bingkai, posisi, serta status selalu-di-atas. Tidak ada jendela yang diubah berdasarkan judul saja, dan tidak ada input atau pesan arbitrer yang dikirim ke aplikasi lain.

Pin mati secara default dan berdiri sendiri dari mode **Biarkan UxPlay**, **Windowed**, atau **Fullscreen**. Perubahan urutan Z memakai `HWND_TOPMOST`/`HWND_NOTOPMOST` bersama `SWP_NOMOVE`, `SWP_NOSIZE`, `SWP_NOACTIVATE`, `SWP_NOOWNERZORDER`, dan posting lintas-thread asinkron, sehingga permintaan tidak mengubah geometri, mengaktifkan target, atau secara langsung memindahkan owner. Windows tetap dapat menempatkan owned window pada band topmost yang sama; iDock tidak menargetkan jendela pengaturan/tray secara langsung. Status dilacak per HWND, direkonsiliasi tanpa membanjiri log, dan diterapkan lagi bila Windows menghapus status topmost. HWND yang dipakai ulang mempertahankan state pelacakannya; jendela pengganti diperlakukan sebagai HWND baru. Menonaktifkan pin menghentikan pemaksaan urutan Z tanpa membatalkan mode tampilan atau ukuran manual. Mekanisme Windows ini berlaku untuk jendela normal dan kebanyakan fullscreen tanpa bingkai, bukan jaminan terhadap exclusive fullscreen, desktop aman, atau jendela topmost lain.

Mulai 0.7.0, `MirrorStreamGeometrySource` memperoleh ukuran stream dari baris *set-caps* `video/x-raw` renderer GStreamer. Tepat sebelum receiver anak dimulai, launcher menambahkan kategori debug sempit D3D11/D3D12 dan file sementara acak khusus sesi hanya jika `GST_DEBUG` maupun `GST_DEBUG_FILE` yang diwarisi tidak berisi nilai. Parser membatasi panjang pembacaan/baris dan memasangkan metadata D3D11 hanya dengan class `GSTD3D11`, serta D3D12 hanya dengan `GstD3D12Hwnd`. File ini tidak berisi tangkapan piksel HP; iDock hanya memakai metadata caps/lifecycle yang dibutuhkan dan menghapus file pada teardown normal.

Ukuran caps baru harus stabil selama 300 ms sebelum diterbitkan. Dalam mode **Windowed**, perubahan bentuk portrait ↔ landscape memicu satu penataan ulang untuk HWND yang sama maupun jendela pengganti. Perubahan caps/resolusi dengan orientasi sama dan perubahan pin tidak memindahkan geometri manual. Jika area klien sudah memiliki bentuk baru, posisi serta ukurannya diterima tanpa penataan ulang. Dengan demikian perubahan ukuran manual bertahan sampai pemilihan mode/**Terapkan ulang** atau perubahan orientasi yang nyata, bukan dilawan pada setiap polling.

Baseline gaya, posisi, dan `WINDOWPLACEMENT` baru direkam ketika iDock pertama kali mengelola tata letak HWND; mengaktifkan pin lebih dahulu tidak membekukannya. Untuk jendela awal yang maximized, iDock meminta `SW_SHOWNOACTIVATE` secara asinkron lalu menunggu siklus berikutnya sebelum menghitung Windowed, sehingga target tidak diaktifkan dan ukuran monitor tidak keliru dianggap sebagai rasio stream fallback. Kembali ke **Biarkan UxPlay** memulihkan placement awal, termasuk keadaan maximized. Pemulihan show state maximized yang tepat memakai `SetWindowPlacement`; berbeda dari operasi pin/normalisasi, Windows dapat mengaktifkan jendela video ketika pengguna secara eksplisit kembali ke mode ini. Rotasi saja tidak menormalisasi jendela yang dimaximize pengguna setelah Windowed aktif; tindakan eksplisit mengganti mode atau **Terapkan ulang** dapat melakukannya.

Jika diagnostik GStreamer kustom sudah diwarisi atau file sementara tidak dapat dibuat, telemetry dimatikan untuk sesi itu dan **Windowed** memakai ukuran area klien awal sebagai fallback. Kegagalan baca sementara juga tidak menggagalkan receiver; fallback berlaku sampai metadata tersedia. Ini mempertahankan diagnostik developer tanpa menghalangi receiver dibuka, tetapi rotasi yang memakai ulang HWND mungkin tidak terdeteksi. Perhitungan tata letak (`MirrorWindowLayout`) dan transisi metadata diuji tanpa jendela nyata.
...
Pemeriksaan otomatis meliputi parsing status/error, penutupan WPF, kepemilikan proses, pengaturan atomik/live reload, keputusan geometri/urutan Z, sensitivitas, rotasi, data tidak valid, mode data path, template kontrol, urutan bagian dalam satu halaman, dan layout ukuran minimum. Hasil tersebut tidak membuktikan kompatibilitas adapter, koneksi perangkat, kualitas radio, penumpukan jendela pada exclusive fullscreen, atau latensi end-to-end. Target penerimaan perangkat nyata ada di [checklist kestabilan](STABILITY-TESTS.md); pengujian installer memiliki [checklist rilis](RELEASING.md) terpisah.
...
iDock for Windows bukan implementasi Apple iPhone Mirroring resmi, bukan toolchain iOS/iPadOS, dan bukan pengganti Xcode debugger. Multi-touch, biometrik, konten terlindungi, pemilihan arah pointer otomatis, serta kontrol bebas latensi tidak dijanjikan. Pelacakan bentuk video otomatis di mode **Windowed** adalah fitur terpisah dengan batas yang dijelaskan di atas.
